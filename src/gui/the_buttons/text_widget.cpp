...
std::vector<glm::uvec4> TextWidget::generate_data() const {
    ...
    for (char character : text_) {
        const render::structures::Character* character_data = font_->get_character(character);
        if (! character_data) {
            LOG_WARNING(logging::main_logger, "Could not find character \"{}\" with value \"{}\" in font. Required for string \"{}\".",
            std::string(1, character), static_cast<int>(character), text_);
        }

        //if (advance + character_data->advance > frame width or character is new_line) { advance = 0; line_advance += line_spacing}
