...
RenderBuffer::connect_texture(GLuint framebuffer_ID, uint8_t texture_attachment) {
    ...
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + texture_attachment, GL_RENDERBUFFER,
        buffer_ID_
    );
}
...
RenderBuffer::connect_depth_texture(GLuint framebuffer_ID) {
    ...
    LOG_BACKTRACE(
        logging::opengl_logger, "Attaching {} to depth texture of framebuffer {}.",
        buffer_ID_, framebuffer_ID
    );
    FrameBufferHandler::instance().bind_fbo(framebuffer_ID);
    glFramebufferRenderbuffer(
        GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, buffer_ID_
    );
}
