...
write_image(T image, const std::filesystem::path& path) {
    ...
    // set information about our image
    png_set_IHDR(
        png_ptr, info_ptr, WIDTH, HEIGHT, 8, PNG_COLOR_TYPE_RGB_ALPHA,
    ...
    // allocate data for row
    row = new (std::nothrow) png_byte[4 * WIDTH];
    if (!row) {
        status = WR_ROW_MALLOC_FAILED;
        goto row_malloc_failed;
    }
    ...
    for (i = 0; i < HEIGHT; i++) {
        // set row data
        for (j = 0; j < WIDTH; j++) {
            const std::array<png_byte, 4> pixel_color = image.get_color(i, j);
            row[4 * j] = pixel_color[0];
            row[4 * j + 1] = pixel_color[1];
            row[4 * j + 2] = pixel_color[2];
            row[4 * j + 3] = pixel_color[3];
        }
