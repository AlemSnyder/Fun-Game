...
class Terrain {
   private:
    ...
    void get_mat_from_qb(const std::map<int, const Material> *materials,
                         uint8_t v[4], Material *&mat_, uint8_t &color_id);
   ...
   public:
    ...
    const UnitPath get_path_type(int xs, int ys, int zs, int xf, int yf,
                                 int zf);

    static float get_H_cost(std::array<float, 3> xyz1,
    ...
    static std::vector<int> generate_macro_map(unsigned int size_x,
                                               unsigned int size_y,
                                               Json::Value map_data);
    void add_to_top(Json::Value to_data,
    ...
    void init(int x, int y, int Area_size_, int z, int seed,
              const std::map<int, const Material> *materials,
              Json::Value biome_data, std::vector<int> Terrain_Maps);
    void init(int x_tiles, int y_tiles, int Area_size_, int z_tiles, int seed,
              int tile_type, const std::map<int, const Material> *material,
              Json::Value biome_data);
    Terrain();
    Terrain(int x_tiles, int y_tiles, int Area_size_, int z_tiles, int seed,
            const std::map<int, const Material> *material,
    ...
    std::set<Node<const T> *> get_adjacent_nodes(
        const Node<const T> *const node,
        std::map<const T *, Node<const T>> &nodes, uint8_t type) const;
    ...
    bool player_set_tile_material(int xyz, const Material *mat,
                                  uint8_t color_id);

    inline void set_tile_material(Tile *tile, const Material *mat,
                                  uint8_t color_id) {
        tile->set_material(mat);
        tile->set_color_id(color_id);
    }

    void set_tile_region(int x_start, int y_start, int z_start, int x_end,
                         int y_end, int z_end, const Material *mat);

    void stamp_tile_region(TileStamp tStamp, int x, int y);
    void stamp_tile_region(int x_start, int y_start, int z_start, int x_end,
                           int y_end, int z_end, const Material *mat,
                           uint8_t color_id);
    // void stamp_tile_region(int x_start, int y_start, int z_start, int x_end,
    // int y_end, int z_end, const Material * mat, std::set<int>
    // elements_can_stamp);
    void stamp_tile_region(int x_start, int y_start, int z_start, int x_end,
                           int y_end, int z_end, const Material *mat,
    ...
    int qb_save_debug(const char *path,
                      const std::map<int, const Material> *materials);
    int qb_save(const char *path) const;
    int qb_read(const char *path,
                const std::map<uint32_t, std::pair<const Material *, uint8_t>>
                    *materials);

    std::set<const NodeGroup *> get_all_node_groups() const;

    std::vector<const Tile *> get_path_Astar(const Tile *start,
                                             const Tile *goal);
    std::vector<const NodeGroup *> get_path_Astar(const NodeGroup *start,
                                                  const NodeGroup *goal);

    std::vector<const Tile *> get_path_breadth_first(
        const Tile *start, const std::set<const Tile *> goal);
    std::vector<const NodeGroup *> get_path_breadth_first(
        const NodeGroup *start,
        const std::set<const NodeGroup *>
        goal);  // TODO this should return a pair
    ...
    int get_first_not(const std::set<std::pair<int, int>> materials, int x,
