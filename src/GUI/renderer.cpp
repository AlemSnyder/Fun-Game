...
MainRenderer::MainRenderer() {
    // non-indexed program
    programID_single_ = load_shaders("../resources/shaders/ShadowMapping.vert",
                                    "../resources/shaders/ShadowMapping.frag");
    // indexed program
    programID_multi_ =
        load_shaders("../resources/shaders/ShadowMappingInstanced.vert",
...
void MainRenderer::add_mesh(
    std::shared_ptr<MeshLoader::SingleComplexMesh> mesh) {
    singles_meshes_.push_back(std::move(mesh));
}

void MainRenderer::add_mesh(
    std::shared_ptr<MeshLoader::MultiComplexMesh> mesh) {
    multis_meshes_.push_back(std::move(mesh));
}

void MainRenderer::set_depth_texture(GLuint texture_id) {
