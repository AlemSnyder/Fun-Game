...
GlobalContext::require_lua_file(const std::string& key, const std::filesystem::path& path, bool create_global) {
    ...
    // in reality should only require the file in one lua state, then copy the generated byte code to all the others
    // this will load onto all states including the one held by the main thread.
    // yes this should be made thread safe, but so should global context
    // It will be fixed before it becomes a problem, just not in this pr (probably Lua-Setup)
...
GlobalContext::GlobalContext() :
    ...
    {
        std::scoped_lock lock(local_context_map_mut);
        local_thread_contexts_.insert({std::this_thread::get_id(), std::move(local_context)});
    }
