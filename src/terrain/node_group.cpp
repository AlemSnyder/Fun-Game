...
void NodeGroup::add_adjacent(NodeGroup* NG, UnitPath type){
    if (NG != this){
...
std::map<const NodeGroup *, UnitPath> NodeGroup::get_adjacent_map() const{
    return std::map<const NodeGroup *, UnitPath>(adjacent.begin(), adjacent.end());
}

std::map<NodeGroup *, UnitPath> NodeGroup::get_adjacent_map(){
    return adjacent;
}
...
std::map<NodeGroup *, UnitPath> NodeGroup::merge_groups(NodeGroup other){
