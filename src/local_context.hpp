...
class LocalContext {
 private:
    ...
    inline int set_arg(size_t i, bool arg) {
        return context_->SetArgByte(i, arg);
    }

    inline int set_arg(size_t i, int8_t arg) {
        return context_->SetArgByte(i, arg);
    }
    inline int set_arg(size_t i, uint8_t arg) {
        return context_->SetArgByte(i, arg);
    }
    inline int set_arg(size_t i, int16_t arg) {
        return context_->SetArgWord(i, arg);
    }
    inline int set_arg(size_t i, uint16_t arg) {
        return context_->SetArgWord(i, arg);
    }
    inline int set_arg(size_t i, int32_t arg) {
        return context_->SetArgDWord(i, arg);
    }
    inline int set_arg(size_t i, uint32_t arg) {
        return context_->SetArgDWord(i, arg);
    }
    inline int set_arg(size_t i, int64_t arg) {
        return context_->SetArgQWord(i, arg);
    }
    inline int set_arg(size_t i, uint64_t arg) {
        return context_->SetArgQWord(i, arg);
    }

    inline int set_arg(size_t i, float arg) {
        return context_->SetArgFloat(i, arg);
    }

    inline int set_arg(size_t i, double arg) {
        return context_->SetArgDouble(i, arg);
    }

    inline int set_arg(size_t i, void* arg) {
        return context_->SetArgObject(i, arg);
    }

    template<typename A>
    inline int set_all_args(const size_t i, const A&& a) {
        int result = set_arg(i, a);
        if (result != 0) {
            LOG_ERROR(logging::main_logger, "An error occurred {} at parameter {}", result, i);
        }
        return result;
    }

    template<typename A, typename... Args>
    inline int set_all_args(const size_t i, const A&& a, const Args&&... args) {
        int result = set_arg(i, a);
        if (result != 0) {
            LOG_ERROR(logging::main_logger, "An error occurred {} at parameter {}", result, i);
            return result;
 ...
 public:
    ...
    template<class... Args>
    inline int
    run_method(asIScriptObject* object, asIScriptFunction* function, const Args&&... args) {
        context_->Prepare(function);
        context_->SetObject(object);
        int result = set_all_args(0, std::forward<const Args>(args)...);
        if (result != 0) {
...
//    template<int>
    int get_return_value(int& value){
        ...
        inline int get_line_number(int stack_level, int* column, const char** section_name) const {
